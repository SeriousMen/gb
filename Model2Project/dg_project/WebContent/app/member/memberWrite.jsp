...
<body class="is-preload" style="font-family: 'paybooc-Bold';">
	...
	<c:choose>
		<c:when test="${session_t_id ne null}">
			<script>
				alert("회원게시물은 회원만 작성할 수 있습니다.");
				location.replace("${pageContext.request.contextPath}/member/MemberBoard.me");
			</script>
		</c:when>
		<c:when test = "${session_m_id eq null and session_t_id eq null}">
			<script>
			alert("로그인 후에 이용해주세요.");
				location.replace("${pageContext.request.contextPath}/app/public/login.jsp");
			</script>
		</c:when>
	</c:choose>
	<!-- Main -->
	<div id="main">
		<div class="wrapper special">
			<div class="inner">
				...
				<h1 style="margin-top: 10%;">게시글 작성하기</h1>
		...
		<div class="wrapper">
			<div class="inner">
				<form method="post"
					action="${pageContext.request.contextPath}/member/MemberWriteOk.me"
					enctype="multipart/form-data" id="m_writeForm" name="m_writeForm">
					<div class="row gtr-uniform">
						<div class="col-6 col-12-xsmall" style="margin: 0 auto;">
							<input type="text" name="photoBoardTitle"
								placeholder="제목을 입력해주세요."> <input type="text"
								name="memberId" id="memberId" value="${session_m_id}"
								readonly="">
							<textarea name="photoBoardContent" id="photoBoardContent"
								rows="6" style="margin-top: 30px; resize: none;"></textarea>
							...
							<p style="color: red;">*이미지가 나오지 않을 시 삭제 후 다시 시도해주세요!</p>



							<ul class="actions fit">
								<li><a href="javascript:memberWriteSubmit()"
									class="button primary fit">게시글 등록</a></li>
							</ul>
						</div>
					</div>
				</form>
...
<script>
	function memberWriteSubmit() {
		var form = document.m_writeForm;
		form.submit();
	}
</script>
<script>
	...
	//첨부 파일 삭제
	/* function cancleFile() {
		var inputTag = document.getElementById('downButton');
		var radios = $("input[name='radioName']");
		for (let i = 0; i < radios.length; i++) {
			if (radios[i].checked) {
