 ...
 *  국민은행 : 입금 시 수수료 50%
	...
	우리은행 : 잔액 조회시 재산 반토막
 */
public class ATM { // 우선은 메소드를 나눠서 객체지향적으로 하지말고 다 main메소드에 풀어서 소스코드를 작성해보기
   public static void main(String[] args) {
      // 각 은행별로 최대 100명의 고객을 유치할 수 있다.
      Shinhan[] arS = new Shinhan[100];	// 클래스 배열을 선언해서 100명의 고객 객체를 담는다.
      Kookmin[] arK = new Kookmin[100];
      Woori[] arW = new Woori[100];

      int[] arMemberCount = { 0, 0, 0 }; //은행이 세개니까 각 은행의 고객 수를 arMemberCount에 담는다?

      Bank[][] arrBank = { arS, arK, arW }; //배열에 배열을 담는거니까 2차원 배열로 선언해야함 Q.이걸왜쓸까 ? 다시 알아보기 
      ...
      Random r = new Random(); //한번만 쓸꺼아니니까 객체화 시킨다. 
      ...
      while (true) {
         ...
         if (bank_choice == 4) {
            break;
            //은행이 늘어나면 은행 개수도 늘어나고 은행메뉴도 늘어난다.
            //개발자가 직접 코드를 수정하지 않고 
            //arMemberCount.length를 통해 컴파일러가 연산하도록 개발한다.  
         }else if (bank_choice <1 || bank_choice>arMemberCount.length) {
        	 continue; //만약 은행을 선택할 떄 목록에 없는 숫자를 입력하면 다시 반복문으로 돌아가 
         }
