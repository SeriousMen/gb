...
public class CommonExceptionAdvice {
	...
	//아래 메소드가 자동으로 실행되어야 하므로 표시를 해주어야 한다.
	@ExceptionHandler(Exception.class)
	public String except(Exception e, Model model) {
		//예외가 발생하게 되면 해당 예외 필드가 메모리에 할당된다.
		//할당된 예외필드의 주소 값을 받을 객체가 필요하므로 매개변수에 Excepion타입의 e 객체를 선언해놓는다.
	...
	@ResponseStatus(HttpStatus.NOT_FOUND)//이거 안해주면 stackoverflow뜬다
